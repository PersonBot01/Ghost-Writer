...
export async function generateChapterPatch(
  request: ChatRequest,
  settings: StoredSettings,
): Promise<ChatResponse> {
  ...
  const parsed = parseStructuredJson<{
    ...
    content?: string
    text?: string
    patch?: { target?: 'selection' | 'chapter'; summary?: string; updatedText?: string } | null
  }>(raw)
  ...
  return {
    ...
    chapterPatch:
      patch && parsed.patch?.summary ? { ...patch, summary: parsed.patch.summary } : patch,
    wikiCreates: [],
    wikiUpdates: [],
    conflicts: [],
...
export async function generateCanonSync(
  ...
  const byName = new Map(wikiDocs.map((entry) => [entry.name.toLowerCase(), entry] as const))
...
async function callOpenRouter(
  ...
  try {
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      throw new Error(
        `OpenRouter request timed out after ${OPENROUTER_TIMEOUT_MS / 1000} seconds. Try a shorter prompt or a faster model.`,
      )
    }
    throw error
  } finally {
...
function selectRelevantWikiEntries(
  entries: WikiDocument[],
  instruction: string,
  sourceText: string,
): WikiDocument[] {
...
function serializeWikiDocs(entries: WikiDocument[]): string {
  return entries
    .map((entry) => {
      const folder =
...
function summarizeConversation(session: ConversationSession | null | undefined): string {
  if (!session) {
    return ''
  }
...
function looksLikeStoryDraft(text: string): boolean {
...
function buildChapterPatch(
  ...
  const target =
    patch.target === 'selection' && selection?.text?.length ? 'selection' : 'chapter'

  if (target === 'selection' && selection) {
    const nextContent =
      sourceText.slice(0, selection.start) +
      patch.updatedText +
...
function buildProposals(
  ...
  projectRoot: string,
  byName: Map<string, WikiDocument>,
  ...
  return items
    .filter(
      (
        ...
        Boolean(item.name && item.description && item.type && WIKI_TYPES.includes(item.type)),
    )
    .map((item) => {
