...
import { appendConversationMessage, createChapter, createConversation, createWikiEntry, deleteNode, ensureProjectStructure, listProjectTree, removeLastAssistantMessage, readChapter, readConversation, readWikiEntry, renameNode, updateWikiEntry, writeChapter, } from './services/project-service.js';
import { DEFAULT_SYSTEM_PROMPT, WIKI_TYPES, } from '../src/shared/types.js';
...
function registerIpc() {
    ...
    ipcMain.handle('project:list-tree', async (_event, rootPath) => {
        return listProjectTree(rootPath);
    });
    ipcMain.handle('chapter:read', async (_event, targetPath) => {
        return readChapter(targetPath);
    });
    ipcMain.handle('chapter:write', async (_event, targetPath, content) => {
        return writeChapter(targetPath, content);
    });
    ipcMain.handle('chapter:write-patch', async (_event, targetPath, nextContent) => {
        return writeChapter(targetPath, nextContent);
    });
    ipcMain.handle('chapter:create', async (_event, rootPath, title) => {
        return createChapter(rootPath, title);
    });
    ipcMain.handle('wiki:read', async (_event, targetPath) => {
        return readWikiEntry(targetPath);
    });
    ipcMain.handle('wiki:create', async (_event, rootPath, type, name, description) => {
        if (!WIKI_TYPES.includes(type)) {
            throw new Error(`Unsupported wiki type: ${type}`);
        }
        return createWikiEntry(rootPath, type, name, description);
    });
    ipcMain.handle('wiki:update', async (_event, targetPath, entry) => {
        return updateWikiEntry(targetPath, entry);
    });
    ipcMain.handle('conversation:create', async (_event, rootPath, title) => {
        return createConversation(rootPath, title);
    });
    ipcMain.handle('conversation:read', async (_event, targetPath) => {
        return readConversation(targetPath);
    });
    ipcMain.handle('conversation:append', async (_event, targetPath, message) => {
        return appendConversationMessage(targetPath, message);
    });
    ipcMain.handle('conversation:remove-last-assistant', async (_event, targetPath) => {
        return removeLastAssistantMessage(targetPath);
    });
    ipcMain.handle('fs:rename', async (_event, targetPath, kind, nextName) => {
