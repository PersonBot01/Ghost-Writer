...
const PROJECT_FOLDERS = ['chapters', 'conversations', 'wiki'];
const WIKI_FOLDERS = {
...
export async function ensureProjectStructure(rootPath) {
    await mkdir(rootPath, { recursive: true });
    await Promise.all(PROJECT_FOLDERS.map(async (folder) => {
        await mkdir(path.join(rootPath, folder), { recursive: true });
    }));
    await Promise.all(Object.values(WIKI_FOLDERS).map(async (folder) => {
...
export async function listProjectTree(rootPath) {
    ...
    root.children = [
        ...
        await buildDirectoryNode(conversationsPath, 'folder', 'Conversations', ['.jsonl'], 'conversation'),
        {
            id: wikiPath,
            kind: 'folder',
            name: 'Wiki',
            path: wikiPath,
            children: await Promise.all(WIKI_TYPES.map(async (type) => {
                const categoryPath = path.join(wikiPath, WIKI_FOLDERS[type]);
                return buildDirectoryNode(categoryPath, 'wikiCategory', capitalize(type) + 's', ['.json'], 'wikiEntry', type);
            })),
        },
    ];
    return root;
}
export async function readChapter(chapterPath) {
    const content = await readFile(chapterPath, 'utf8');
    return { path: chapterPath, content };
}
export async function writeChapter(chapterPath, content) {
    await writeFile(chapterPath, content, 'utf8');
    return { path: chapterPath, content };
}
export async function createChapter(rootPath, title) {
    const normalizedTitle = title.trim() || 'Untitled Chapter';
    const filePath = await createUniquePath(path.join(rootPath, 'chapters'), slugify(normalizedTitle) || 'untitled-chapter', '.md');
    const content = `# ${normalizedTitle}\n\n`;
    await writeFile(filePath, content, { encoding: 'utf8', flag: 'wx' });
    return { path: filePath, content };
}
export async function readWikiEntry(entryPath) {
    const raw = await readFile(entryPath, 'utf8');
    const entry = wikiEntrySchema.parse(JSON.parse(raw));
    return { path: entryPath, ...entry };
}
export async function createWikiEntry(rootPath, type, name, description = '') {
    const entry = {
        name: name.trim(),
        type,
        description,
...
export async function updateWikiEntry(entryPath, entry) {
    wikiEntrySchema.parse(entry);
    await writeFile(entryPath, `${JSON.stringify(entry, null, 2)}\n`, 'utf8');
    return { path: entryPath, ...entry };
}
export async function listWikiDocuments(rootPath) {
    const documents = await Promise.all(WIKI_TYPES.map(async (type) => {
...
export async function createConversation(rootPath, title) {
    const normalizedTitle = title.trim() || 'New conversation';
    const timestamp = new Date().toISOString();
    const filePath = path.join(rootPath, 'conversations', `${timestamp.slice(0, 19).replace(/[:T]/g, '-')}-${slugify(normalizedTitle)}.jsonl`);
    const meta = {
        kind: 'session',
...
export async function readConversation(conversationPath) {
    ...
    const messages = lines
        .slice(1)
        .map((line) => conversationMessageSchema.parse(JSON.parse(line)));
    return {
...
export async function appendConversationMessage(conversationPath, message) {
...
export async function removeLastAssistantMessage(conversationPath) {
    const session = await readConversation(conversationPath);
...
export async function renameNode(targetPath, kind, nextName) {
    ...
    if (kind === 'chapter') {
        ...
        const renamedEntry = { ...document, name: baseName };
        ...
        nextPath = path.join(directory, `${session.meta.createdAt.slice(0, 19).replace(/[:T]/g, '-')}-${slugify(baseName)}.jsonl`);
        await rename(targetPath, nextPath);
...
export async function deleteNode(targetPath) {
...
export function resolveWikiPath(rootPath, entry) {
...
async function buildDirectoryNode(folderPath, kind, label, extensions, childKind, wikiType) {
...
async function safeReadDirectory(folderPath) {
    try {
...
async function writeConversationSession(session) {
    const lines = [
...
async function createUniquePath(directory, baseName, extension) {
    let attempt = 0;
    while (attempt < 500) {
...
function trimConversationLabel(fileName) {
    return fileName
